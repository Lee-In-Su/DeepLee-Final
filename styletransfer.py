...
class StyleFrame:
    ...
    def content_loss(self, content):
        ...
        return hub.sum(hub.square(content - self))
    ...
    def get_input_frames(self):
        ...
        image = cv2.resize(image,  (self.frame_width, self.conf.FRAME_HEIGHT), interpolation=INTER_CUBIC)
    ...
    def get_style_info(self):
        ...
        # 첫 번째 스타일 파일을 열고 다른 모든 스타일 파일크기와 일치시킨다.
        ...
        for filename in style_files:
            ...
            style_ref = cv2.cvtColor(style_ref, cv2.COLOR_RGB2BGR)
            ...
            # 첫 번째 style_ref 치수에 맞게 모든 style_ref 이미지 크기 조정
            if style_ref_width != first_style_width or style_ref_height != first_style_height:
                resized_ref = True

                style_ref = cv2.resize(style_ref, (first_style_width, first_style_height), interpolation=INTER_CUBIC)
    ...
    def get_output_frames(self):
        ...
        for count, filename in enumerate(sorted(self.input_frame_directory)):
            ...
            content_img = tf.cast(tf.convert_to_tensor(content_img), tf.float32)
            # output_img = self.style_transfer_model.transform(content_img, blended_img)
            # output_img = tf.cast(tf.convert_to_tensor(output_img), tf.float32)
            ...
            temp_ghost_frame = cv2.cvtColor(ghost_frame, cv2.COLOR_BGR2RGB) * self.MAX_CHANNEL_INTENSITY
            cv2.imwrite(self.conf.OUTPUT_FRAME_PATH.format(count), temp_ghost_frame)
        self.output_frame_directory = glob.glob(f'{self.conf.OUTPUT_FRAME_DIRECTORY}/*')

    def _color_correct_to_input(self, content, generated):

        # opencv와의 호환성을 위한 이미지 조작
        ...
        content = self._trim_img(content)

        # 채널 추출, 강도 및 색 공간 병합
        
        content_y = cv2.cvtColor(content, cv2.COLOR_RGB2BGR)[:,:,0]
        content_cb = cv2.cvtColor(content, cv2.COLOR_RGB2BGR)[:,:,1]
        content_cr = cv2.cvtColor(content, cv2.COLOR_RGB2BGR)[:,:,2]
        generated_y = cv2.cvtColor(generated, cv2.COLOR_RGB2BGR)[:,:,0]
        generated_cb = cv2.cvtColor(generated, cv2.COLOR_RGB2BGR)[:,:,1]
        generated_cr = cv2.cvtColor(generated, cv2.COLOR_RGB2BGR)[:,:,2]
        ...
        # # convert back to BGR
        # color_corrected = cv2.cvtColor(color_corrected, cv2.COLOR_YCR_CB2BGR)
        # color_corrected = color_corrected / self.MAX_CHANNEL_INTENSITY
